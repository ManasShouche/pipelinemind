...
**MCP primitives:** Tools (6) · Resource (schema drift, polled every 300 s) · Prompt (`/diagnose_pipeline {id}`)
...
All configuration is in `.env` (gitignored). Copy `.env.example` to start.
...
```
pipelinemind/
├── agent/                    # LLM router · agent loop · MCP server · tools
├── api/                      # FastAPI backend (SSE · routers · middleware)
├── ingestion/                # Chunkers · embedders · summary generator
├── retrieval/                # HyDE · ChromaDB · BM25 · RRF · reranker · graph augmentor
├── ui/                       # Streamlit app (chat · health · catalogue)
├── db/                       # DuckDB schema + seeder
├── data/                     # Synthetic fixtures (pipelines · SQL · DAGs · dbt · logs)
├── scripts/                  # start_api · start_ui · ingest · seed
├── tests/                    # Unit · integration · RAG eval
├── docs/                     # INTERNALS · SPEC · CHANGES_DETAILED
└── spec/                     # FUNCTIONAL_SPEC · TECHNICAL_SPEC
```
...
# Smoke test — verifies full SSE trace pipeline end-to-end (no API key needed)
PYTHONPATH=. python scripts/smoke_trace.py
```
...
The app ships as two Docker containers (`api` + `ui`) orchestrated by `docker-compose.yml`.

**EC2 (recommended for demo):**
```bash
# On EC2 t3.large (Ubuntu 22.04)
sudo apt install -y docker.io docker-compose-plugin
docker compose up --build -d
```
Security group: open port `8501` publicly, keep `8000` internal.
...
| Change log (engineering) | `docs/CHANGES_DETAILED.md` |
| Full specification | `docs/SPEC.md` |
| Functional spec | `spec/FUNCTIONAL_SPEC.md` |
| Technical spec | `spec/TECHNICAL_SPEC.md` |
